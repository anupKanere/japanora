...
import { EssentialsQuizModal } from '@/components/reference/EssentialsQuizModal'
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="animate-fade-in" key={activeTab}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 mb-4 border-b border-border/60">
          ...
          <button
            type="button"
            onClick={() => setQuizTab(activeTab)}
            className="self-start sm:self-auto px-4 py-2 rounded-xl bg-accent text-white hover:bg-accent/90 shadow-md flex items-center gap-2 text-xs font-bold transition-all active:scale-95 group cursor-pointer"
            title={`Practice all ${currentTab?.count} ${currentTab?.label} words`}
          >
            <Flame size={15} className="group-hover:scale-110 text-amber-300 transition-transform" />
            <span>Practice Quiz ({currentTab?.count} words)</span>
      ...
      {/* Essentials Practice Speed Quiz Modal */}
      {quizTab && (
        <EssentialsQuizModal
          tabId={quizTab}
          onClose={() => setQuizTab(null)}
        />
      )}
    </div>
  )
